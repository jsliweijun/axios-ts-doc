...
```bash
git clone https://github.com/alexjoverm/typescript-library-starter.git axios-ts
cd axios-ts

npm install
```

先通过 `git clone` 把项目代码拉下来到我们的 `axios-ts` 目录，然后运行 `npm install` 安装依赖，并且给项目命名，我们仍然使用 `axios-ts`。
